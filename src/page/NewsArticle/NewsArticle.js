...
const stateToProps = state => ({
    watchArticle: state.watchArticleID === -1 ? null : state.newsList[state.newsList.length - state.watchArticleID]
});
...
class NewsArticle extends Component {
    ...
    render() {
        if (this.props.watchArticle === null) return <Redirect to="/news"/>
            ...
            <div className="article-content" dangerouslySetInnerHTML={{__html: this.props.watchArticle.content}}></div>
        </div>);
    }
}
